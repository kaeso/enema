...
class QueryEditorForm(QtWidgets.QWidget):
    ...
    def cloneSettings(self, settings, newPath):
        cloned_settings = QtCore.QSettings(newPath, QtCore.QSettings.Format.IniFormat)
    ...
    def replaceAndSave(self, save_only=False):
        ...
        if stringsFound <= 0:
            QtWidgets.QMessageBox.information(self, "Enema - Query Editor", "Nothing to replace.", 1, 0)
            return
        else:
            QtWidgets.QMessageBox.information(self, "Enema - Query Editor", "Replaced: " + str(stringsFound), 1, 0)
    ...
    def loadQstrings(self):
        ...
        self.settings = QtCore.QSettings(path, QtCore.QSettings.Format.IniFormat)
        ...
        if not self.edited:
            ...
            for group in self.settings.childGroups():
                ...
                for key in self.settings.childKeys():
                    grp_child =  QtWidgets.QTreeWidgetItem(grp)
...
class EncoderForm(QtWidgets.QWidget):
...
#Preferences widget
class PreferencesForm(QtWidgets.QWidget):
...
#Main form GUI class
class EnemaForm(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        #System tray icon
        self.sysTray=QtWidgets.QSystemTrayIcon(trayIcon, self)
        ...
        self.ui.treeOfTables.header().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
...
#------------------------------------------------PLUGINS------------------------------------------------------#
    ...
    def addPluginAction(self, plugin_group, plugin_name, plugin_class):
        ...
        if len(self.plugin_groups) < 1:
            self.p_group = QtWidgets.QMenu(plugin_group.lower(), self)
    ...
    def pluginSlotConnect(self, p_action,  plugin_class):
        ...
        signalMapper.mappedString.connect(self.pluginAction_Triggered)
...
#------------------------------------------------GENERAL-FUNCTIONS------------------------------------------------------#
    ...
    def connectAndStart(self, blind=False):
        if not blind:
            #dbSignal
            self.qthread.dbSignal.connect(self.addBase, type=QtCore.Qt.ConnectionType.QueuedConnection)
            #columnSignal
            self.qthread.columnSignal.connect(self.addColumn, type=QtCore.Qt.ConnectionType.QueuedConnection)
            #rowDataSignal
            self.qthread.rowDataSignal.connect(self.addRowData, type=QtCore.Qt.ConnectionType.QueuedConnection)
            #tblSignal
            self.qthread.tblSignal.connect(self.addTable, type=QtCore.Qt.ConnectionType.QueuedConnection)
        else:
            #Setting 'True' time
            self.qthread.trueTimeSignal.connect(self.setTrueTime, type=QtCore.Qt.ConnectionType.QueuedConnection)
            
        #logSignal
        self.qthread.logSignal.connect(self.addLog, type=QtCore.Qt.ConnectionType.QueuedConnection)
        #progressSignal
        self.qthread.progressSignal.connect(self.updatePb, type=QtCore.Qt.ConnectionType.QueuedConnection)
        #querySignal
        self.qthread.querySignal.connect(self.queryResult, type=QtCore.Qt.ConnectionType.QueuedConnection)
...
#------------------------------------------------[MENU]SAVE/FUNCTIONS------------------------------------------------------#
    ...
    def saveTables_OnClick(self):
        filePath = QtWidgets.QFileDialog.getSaveFileName(self, "Save tables",
                                                     QtCore.QDir.homePath(),
                                                     ("Text files (*.txt)"))
        self.writeToFile(filePath[0], "tables")
        
    #Click on menu save columns 
    def saveColumns_OnClick(self):
        filePath = QtWidgets.QFileDialog.getSaveFileName(self, "Save columns", 
                                                     QtCore.QDir.homePath(),
                                                     ("Text files (*.txt)"))
        self.writeToFile(filePath[0], "columns")
        
    #Click on menu save bases  
    def saveBases_OnClick(self):
        filePath = QtWidgets.QFileDialog.getSaveFileName(self, "Save bases",
                                                     QtCore.QDir.homePath(),
                                                     ("Text files (*.txt)"))
        self.writeToFile(filePath[0], "bases")

    #Click on menu csv export
    def csvExport_OnClick(self):
        filePath = QtWidgets.QFileDialog.getSaveFileName(self, "Export to csv",
                                                     QtCore.QDir.homePath(),
                                                     ("CSV files (*.csv)"))
        self.writeToFile(filePath[0],  "csv")
        
    #Click on menu save site settings
    def saveSiteSettings_OnClick(self):
        filePath = QtWidgets.QFileDialog.getSaveFileName(self, "Save site settings",
                                                     QtCore.QDir.homePath(),
                                                     ("INI files (*.ini)"))
    ...
    def saveSiteSettings(self, filepath):
        settings = QtCore.QSettings(filepath[0], QtCore.QSettings.Format.IniFormat)
...
#------------------------------------------------[MENU]LOAD-SLOTS/FUNCTIONS------------------------------------------------------#
    ...
    def loadTables_OnClick(self):
        filePath = QtWidgets.QFileDialog.getOpenFileName(self, "Load tables", 
                                                     QtCore.QDir.homePath(),
                                                     ("Text files (*.txt)"))
        self.readFromFile(filePath[0], "tables")
        
    #Click on menu load bases  
    def loadBases_OnClick(self):
        filePath = QtWidgets.QFileDialog.getOpenFileName(self, "Load bases", 
                                                     QtCore.QDir.homePath(),
                                                     ("Text files (*.txt)"))
        self.readFromFile(filePath[0], "bases")
        
    #Click on menu load site settings  
    def loadSiteSettings_OnClick(self):
        filePath = QtWidgets.QFileDialog.getOpenFileName(self, "Load site settings", 
                                                     QtCore.QDir.homePath(),
                                                     ("INI files (*.ini)"))
...
#------------------------------------------------[MENU]TOOLS-SLOTS------------------------------------------------------#
    ...
    def loadQstrings(self):
        ...
        qstrings = QtCore.QSettings(qs_path, QtCore.QSettings.Format.IniFormat)
    ...
    def runButton_OnClick(self):
        ...
        if self.ui.radioColumns.isChecked():
            ...
            if self.ui.treeOfTables.topLevelItemCount() < 1:
                QtWidgets.QMessageBox.information(self, "Enema", "Drag and drop table from left field to right.", 1, 0)
                return
            tables = []
...
#----------------------------------------------MAIN-EVENTS----------------------------------------------------#
    ...
    def closeEvent(self, event):
        if self.sysTray.isSystemTrayAvailable():
            self.hide()
            if self.firstHide:
                #self.sysTray.showMessage("Enema", "I'll wait here...")
